    ...
    def cb_retrieve_pre_computed(
        ...
        Args:
            ...
            gpu_block_ids: One block table per engine group, indexed by
                ``EngineGroupInfo.engine_group_id`` (extra-object groups
                follow the regular groups; kernel groups resolve theirs via
                ``engine_group_idx``, and a missing table is an error).
                Unlike the engine RETRIEVE contract (table fully allocated
                for an external prefix hit), the table here may be PARTIALLY
                allocated: vLLM calls once per block-alloc round, and it only
                ever grows at the tail — matches beyond the allocated bound
                defer or fail the scatter per the all-or-nothing rule above.
